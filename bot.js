...
async function handlePlay(interaction) {
    ...
    try {
        ...
        if (songInput.includes('youtube.com') || songInput.includes('youtu.be')) {
            ...
            console.log(`🔍 Searching YouTube for: ${songInput}`);
            ...
            try {
                ...
                if (!videoResult || !videoResult.videos || !videoResult.videos.length) {
                    console.log('❌ No results from yt-search');
                    return interaction.editReply('💔 No melodies found! Try another search or use a YouTube URL!');
                }
                ...
                console.log(`✅ Found via search: ${song.title}`);
            } catch (searchError) {
                console.error('Search error:', searchError.message);
                // If search fails, suggest using URL
                return interaction.editReply(
                    '💫 Search is having trouble right now! Please use a YouTube URL instead!\n' +
        ...
        return interaction.editReply(
            '💫 There was an error! Please try using a YouTube URL!\n' +
            'Example: `/play https://www.youtube.com/watch?v=dQw4w9WgXcQ`'
        );
    }
...
async function playSong(guild, song) {
    ...
    try {
        console.log(`🎵 Starting playback: ${song.title}`);
        ...
        // Set volume to 50% (prevents distortion)
        if (resource.volume) {
        ...
        serverQueue.player.on(AudioPlayerStatus.Idle, async () => {
            console.log('⏭️ Song finished, playing next...');
        ...
        serverQueue.player.on(AudioPlayerStatus.Paused, () => {
            console.log('⏸️ Player status: Paused');
        });
        ...
        serverQueue.textChannel.send(`💫 There was an error playing this song! Trying next...`);
        serverQueue.songs.shift();
        await playSong(guild, serverQueue.songs[0]);
    }
}
